...
class Block:
    ...
    def adjust_difficulty(last_block, new_timestamp):
        """
        Caculate the adjusted difficulty to the MINE_RATE
        Increase the difficulty for quickly mined blocks
        Decrease the difficulty fro slowly mined blocks
        """

        if (new_timestamp - last_block.timestamp) < MINE_RATE:
            return last_block.difficulty + 1
        elif last_block.difficulty > 1:
            return last_block.difficulty - 1
        else:
            return 1
    ...
    def is_valid_block(last_block, block):
        ...
        if block.last_hash != last_block.hash:
            raise Exception(f'The block last_hash must be correct')
        ...
        reconstructred_hash = crypto_hash(
            block.last_hash, 
            block.data, 
            block.timestamp, 
            block.nonce, 
